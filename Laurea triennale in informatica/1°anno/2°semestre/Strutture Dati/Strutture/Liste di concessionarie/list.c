...
static struct node* makeNode(Concessionaria val, struct node* nxt);
...
int posConcessionaria(list l,Concessionaria val)
{
	...
	int pos=0;    // mi salva la posizione dell'Concessionaria 
	int found=0;  //viene settata ad 1 se l'Concessionaria è stato trovato

	struct node *temp= l->first;  //il puntatore temp punta al primo elemento della lista l

	while(temp!=NULL && !found)
	   {
		if(eq1(temp->value,val)==0)
		   {
...
Concessionaria searchConcessionaria(list l, Concessionaria x)
{
	...
	while(temp!=NULL)
	   {
		if(eq1(temp->value,x)==0)
		   {
 			return temp->value;    //elemento trovato
		   }
		else
		  {
			temp=temp->next;    //temp punta al prossimo elemento e continua la ricerca nella lista
...
{
	if(l==NULL)          //se l non esiste restituisco nullConcessionaria
	  {
		return nullconcessionaria;
	  }
   
	if(pos<0||pos>=l->size)   //se la posizione inserita è minore di 0 o piu grande della grandzza della lista restituisco NULLConcessionaria
	 {
		return nullconcessionaria;   // precondizione non verificata
	 }	   
	...
	while(temp != NULL)
	  {
		...
		output_Concessionaria(temp->value);
		printf("\n");
...
static struct node* insertNode (struct node* n, int pos, Concessionaria val)
{
	struct node *n1;
...
{
	int pos=posConcessionaria(l,x)+1;
