...
plt.plot(U3[:,0], U3[:,1], 'bo', markersize=15, clip_on=False, label='seasons')
plt.plot(V3[:,0], V3[:,1], 'rs', markersize=15, clip_on=False, label='users')

ax = plt.gca()
for i, txt in enumerate(seasons):
    ax.text(U3[i,0], U3[i,1], txt, ha='left', va='bottom', fontsize=20)

for i, txt in enumerate(users):
    ax.text(V3[i,0], V3[i,1], txt, ha='left', va='bottom', fontsize=20)
...
luke2d = luke.dot(U3.dot(np.linalg.inv(S3)))
