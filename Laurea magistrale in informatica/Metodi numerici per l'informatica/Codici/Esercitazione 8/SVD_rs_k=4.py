...
plt.plot(U4[:,0], U4[:,1], 'bo', markersize=15, clip_on=False, label='seasons')
plt.plot(V4[:,0], V4[:,1], 'rs', markersize=15, clip_on=False, label='users')

ax = plt.gca()
for i, txt in enumerate(seasons):
    ax.text(U4[i,0], U4[i,1], txt, ha='left', va='bottom', fontsize=20)

for i, txt in enumerate(users):
    ax.text(V4[i,0], V4[i,1], txt, ha='left', va='bottom', fontsize=20)
...
luke2d = luke.dot(U4.dot(np.linalg.inv(S4)))
