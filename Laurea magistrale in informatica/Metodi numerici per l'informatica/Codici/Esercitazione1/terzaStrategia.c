/* --------------------------------------------------------------------------
  ...
  con p processori, con p potenza di 2, usando a terza strategia.
----------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------
...
int main(int argc, char **argv) {
    ...
    int ind, p, r, sendTo, recvBy, sendReceive;
    int *potenze, passi = 0;
    float sommaloc = 0;
    ...
    if (idProc == 0) {
        ...
        for (i = 1; i < nproc; i++) {
            tag =
                i; /* tag del messaggio uguale all'id del processo che riceve*/
            ...
            if (i < resto) {
                ...
                /*il processore P0 gli invia il corrispondete vettore locale*/
                MPI_Send(vett + ind, nlocgen, MPI_FLOAT, i, tag,
                         MPI_COMM_WORLD);
                ind = ind + nlocgen;
            }  // end if
        }      // end for
    ...
    /* comunicazioni: se resto(menum,2k+1)< 2k comunico con + dist altrimenti con -dist */
    for (i = 0; i < passi; i++) {
        // ... calcolo identificativo del processore
        r = idProc % potenze[i + 1];

        // Se il resto è minore  a 2^i, il processore idProc invia e rceve i dati
        if (r < potenze[i]) {
            // calcolo dell'id del processore a cui spedire la somma locale
            sendReceive = idProc + potenze[i];
            tag = sendReceive;
            MPI_Send(&sommaloc, 1, MPI_FLOAT, sendReceive, tag, MPI_COMM_WORLD);
            MPI_Recv(&tmp, 1, MPI_FLOAT, sendReceive, tag, MPI_COMM_WORLD, &info);

            sommaloc = sommaloc + tmp;
        } else  // se il resto � uguale a 0, il processore idProc riceve
        {
            sendReceive = idProc - potenze[i];
            tag = idProc;
            MPI_Send(&sommaloc, 1, MPI_FLOAT, sendReceive, tag, MPI_COMM_WORLD);
